...
function Banco:carregarOuCriar()
    ...
    if file then
        for line in file:lines() do -- para cada linha existente
            local usuario, senha, saldo = line:match("([^,]+),([^,]+),([^,]+)") -- padrao de procura
...
function Banco:criarConta()
    io.write("Digite o seu nome de usuario: ")
    ...
    self:salvarContas() -- ira salvar, pois ja esta na tabela
